...
* **Description:** Filters tool contributions and hooks based on `repo.required_plugin_ids` (with `publish_api` forced as infrastructure). Merges environment variables and scans `MAYA_PLUG_IN_PATH` directories for plugins (`.py`, `.mll`, `.pyd`, `.so`) to force-load.
